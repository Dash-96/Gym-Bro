...
import { queryClient } from "../serverStateStore/queryClient";
import { getSocketConnection } from "./webSocket";

export async function testConnection() {
  const socketConnection = getSocketConnection();
  try {
    await socketConnection.invoke("SendFriendRequest", 42);
  } catch (error) {
    console.log(error);
  }
}
...
export function useFriendRequestNotifications() {
  ...
  function notificationHanlder(payload: { senderName: string; message: string }) {
    console.log("reciver notification");
  ...
  // socketConnection.off("friendRequest", notificationHanlder);
}
