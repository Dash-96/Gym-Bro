  ...
  await session.withTransactionAsync(async () => {
    try {
      const workoutStatement = await session.prepareAsync(
        "UPDATE workouts SET workout_type = $workoutType, workout_alias = $workoutAlias, notes = $notes, started_at = $startedAt , finished_at = $finishedAt , updated_at = datetime('now') WHERE id = $id",
      );
